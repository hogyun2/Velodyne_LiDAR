...
class get_odo:
    def __init__(self):
        ...
        rospy.Subscriber("/Imu", Imu, self.getIMU)
    ...
    def getIMU(self, msg):
        ...
        self.heading = round(self.heading%360) - 90
    ...
    def cal_steering(self, cur_x, cur_y, cur_yaw):
        ...
        self.goal_x = self.map.points[(min_index+self.lookahead) % (len(self.map.points)-1)].x
        self.goal_y = self.map.points[(min_index+self.lookahead) % (len(self.map.points)-1)].y
